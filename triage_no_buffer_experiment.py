...
# triage_no_buffer_experiment.py
import simpy
import random
import statistics
import csv
...
def patient(env, pid, prep_res, theatre_res, recovery_res, stats):
    ...
    # Тепер, коли theatre отримано, RELEASE prep (пацієнт йде в theatre)
    ...
    # --- REQUEST RECOVERY ТА БЛОКУВАННЯ (утримуючи theatre) ---
    block_start = None
    if recovery_res.count >= recovery_res.capacity:
        block_start = env.now  # почалося блокування операційної (чекає на recovery)
    ...
    # Якщо було блокування, обчислити тривалість
    if block_start is not None:
        stats['total_blocked_time'] += (env.now - block_start)
        stats['blocking_events'] += 1

    # Release theatre (пацієнт перейшов у recovery)
...
# ====== МОНИТОР (семплування після warm-up) ======
...
# ====== ПРОГАНЯТИ СЕРІЮ ДЛЯ ВСІХ КОНФІГІРАЦІЙ (і зберегти CSV) ======
